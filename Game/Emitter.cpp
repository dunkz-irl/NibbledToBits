...
Particle Emitter::ChooseParticleLocationAndInitialVelocity()
{
	...
	if (!m_particleLine)
	{
		...
		float randAngle = RandomFloat(DegToRads(m_minEmissionDirection), DegToRads(m_maxEmissionDirection)); // (i.e. theta in a right angled triangle)
...
void Emitter::CreateParticle()
{
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	//
	//	pointer - level of indirection - quite inefficient
	//	Jake's addresses around Sheffield analogy - much better to have a dynamically sized array that is all in one place
	//	Pointers can be anywhere, and are hence inefficient to access
	// 
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
...
void Emitter::SetMouseFollow(bool state)
{
	m_followCursor = state;
}
...
void Emitter::Update(float elapsedTime)
{
	...
	for (Particle& p : v_Particles)
	{
		...
		if (m_particleLine)
		{
			...
			Play::Vector2f unitPerpVec = Play::normalize({ lineVec.y, -lineVec.x }); // This is how you get a perpendicular vector apparently
		...
		{
			Play::Vector2f facingUnitVect = RotateVector({ 0.f, 1.f }, p.rotation);
		...
		if (m_directionWiggle != 0.0f)
		{
			...
			if (m_velocityInOrientationDirection)
			{
				p.rotation = AngleBetweenTwoVectors({ 0.f, 1.f }, p.velocity);
			}
		}
		...
		if (m_useGravity)
		{
			...
			{
			case UP:
				p.velocity += PARTICLE_GRAVITY * m_gravityMultiplier;
				break;
	...
	while (it != v_Particles.end())
	{
		if (!it->isActive) // Is an iterator is it a pointer precious????
			it = v_Particles.erase(it); // erase returns an iterator immediately after the one that was erased: https://stackoverflow.com/questions/4713131/removing-item-from-vector-while-iterating
		else
			++it;
	}
}
...
void Emitter::SetEmitContinuously(bool state)
{
	m_emitContinuously = state;
	m_spawnTimer = 0.f;
}
